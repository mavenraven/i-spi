...
func statementUsesIdent(stmt ast.Stmt, ident string) bool {
	...
	case *ast.BasicLit:
		return false
	case *ast.FuncLit:
		funcLit := stmt.(*ast.FuncLit)
		for _, param := range funcLit.Type.Params.List {
			for _, name := range param.Names {
				if name.Name == ident {
					// The function literal shadows the identifier.
					return false
				}
			}
		}

		return statementUsesIdent(funcLit.Body, ident)
	case *ast.CompositeLit:
		compositeLit := stmt.(*ast.CompositeLit)
		if compositeLit.Elts == nil {
			return false
		}

		for _, expr := range compositeLit.Elts {
			if statementUsesIdent(expr, ident) {
				return true
			}
		}
		...
		return false
	case *ast.ParenExpr:
		parenExpr := stmt.(*ast.ParenExpr)
		return statementUsesIdent(parenExpr.X, ident)
	case *ast.SelectorExpr:
		selectorExpr := stmt.(*ast.SelectorExpr)
		return statementUsesIdent(selectorExpr.X, ident)
	case *ast.IndexExpr:
		indexExpr := stmt.(*ast.IndexExpr)
		return statementUsesIdent(indexExpr.X, ident)
	case *ast.SliceExpr:
		sliceExpr := stmt.(*ast.SliceExpr)
		return statementUsesIdent(sliceExpr.X, ident)
	case *ast.TypeAssertExpr:
		typeAssertExpr := stmt.(*ast.TypeAssertExpr)
		return statementUsesIdent(typeAssertExpr.X, ident)
	case *ast.CallExpr:
		callExpr := stmt.(*ast.CallExpr)
		callExpr
	case *ast.StarExpr:
	case *ast.UnaryExpr:
	case *ast.BinaryExpr:
